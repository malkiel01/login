...
try {
    switch ($action) {
        ...
        // רשימת רכישות ממתינות לאישור
        case 'listPending':
            requireViewPermission('purchases');

            $currentUserId = getCurrentUserId();

            // שליפת בקשות ממתינות לרכישות
            $sql = "
                SELECT
                ...
                WHERE peo.entity_type = 'purchases'
            ...
            foreach ($pendingList as &$pending) {
                ...
                $pending['price'] = $operationData['price'] ?? 0;
                $pending['buyer_status'] = $operationData['buyer_status'] ?? '';
                $pending['purchaseStatus'] = $operationData['purchaseStatus'] ?? '';
