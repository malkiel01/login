...
const FormHandler = {
        // פונקציה עזר אחידה לחיכוי לאלמנטים
    ...
    // פונקציה עזר לחיכוי לפונקציות גלובליות
        ...
        try {
            ...
            if (modal) {
                ...
                if (type === 'customer') {
                    ...
                    // יצירת observer שיחכה לטעינת הסקריפט
                    const observer = new MutationObserver((mutations, obs) => {
                        // בדוק אם הפונקציות זמינות
                        if (window.hierarchyData && typeof window.populateBlocks === 'function') {
                            // הפונקציות זמינות! אתחל אותן
                            window.populateBlocks();
                            window.populatePlots();
                            
                            // שלח אירוע למקרה שמשהו מקשיב
                    ...
                    // התחל לצפות בשינויים ב-head (שם נוספים הסקריפטים)
                    observer.observe(document.head, {
                        childList: true,
                        subtree: true
                    });
                    
                    // הגבלת זמן - אחרי 5 שניות תפסיק לנסות
                    setTimeout(() => {
                        observer.disconnect();
                        console.error('Purchase form functions did not load in time');
                    }, 5000);
                }
                ...
                if (itemId && type !== 'customer' && type !== 'purchase') {
