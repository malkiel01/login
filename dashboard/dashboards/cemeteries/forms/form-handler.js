...
const FormHandler = {
    ...
    handleCustomerForm3: function(itemId) {
        // המתן ל-address-fieldset
        this.waitForElement('#address-fieldset', (fieldset) => {
            
            // אתחל SmartSelect אם קיים
            ...
            // קבל נתוני ערים
            if (!fieldset.dataset.cities) return;
            const citiesData = JSON.parse(fieldset.dataset.cities);
            ...
            // טיפול ב-Select רגיל (גיבוי אחורה)
            else {
                window.filterCities = function() {
                    const countrySelect = document.getElementById('countrySelect');
                    const citySelect = document.getElementById('citySelect');
                            ...
                            Object.keys(result.data).forEach(key => {
                                ...
                                if (field) {
                                    if (field.type === 'checkbox') {
                                        ...
                                        // אם זה שדה מדינה - טען גם ערים
                                        if (key === 'countryId') {
                                            // אם יש SmartSelect
                                            if (window.SmartSelectManager && window.SmartSelectManager.instances['cityId']) {
                                                // טריגר את event change
                                                ...
                                                // אחרי שהערים נטענו, בחר את העיר הנכונה
                                                setTimeout(() => {
