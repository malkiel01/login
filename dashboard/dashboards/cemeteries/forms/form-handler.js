...
const FormHandler = {
    ...
    handlePurchaseForm2: function(itemId) {
        // המתן שהמודל יטען
        this.waitForElement('#purchaseFormModal', (modal) => {
            // קרא את הנתונים מה-modal
            const hierarchyData = modal.dataset.hierarchy;
            if (hierarchyData) {
                window.hierarchyData = JSON.parse(hierarchyData);
            } else {
                console.error('hierarchyData not found');
                return;
            }
            
            // בדוק אם יש את הנתונים
    ...
    handlePurchaseForm: function(itemId) {
        ...
        this.waitForElement('#grave-selector-fieldset', (fieldset) => {
            ...
            // כל שאר הקוד שלך כמו שהוא (מחק את הבדיקה הכפולה בשורות 294-298)
