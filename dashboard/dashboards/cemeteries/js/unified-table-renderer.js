...
// // שורות 866-884 בקובץ unified-table-renderer.js
// // ==========================================

// window.loadAllCemeteries = async function() {
//     console.log('📍 Loading all cemeteries - NEW VERSION');
...
//     // טען עם UniversalSearch
//     await initCemeteriesUniversalSearch();
// };

// // ==========================================
// // 🆕 הוסף את הפונקציות האלה לפני סוף הקובץ
// // (לפני הסגירה האחרונה)
// // ==========================================

// /**
//  * בניית container לבתי עלמין
//  */
...
// console.log('✅ Cemeteries functions added to unified-table-renderer.js');


/**
 * תיקון בעיית טעינת בתי עלמין
 * 
 * הבעיה: מישהו ניסה להוסיף UniversalSearch אבל לא השלים את ההטמעה
 * והקוד הישן של unified-table-renderer.js נמחק או נפגע
 * 
 * פתרון: החלפת הפונקציה הבעייתית בטעינה רגילה
 */

// החלפת הפונקציה הבעייתית - חדש ומתוקן!
window.loadAllCemeteries = async function() {
    console.log('📍 Loading all cemeteries - NEW VERSION');
    ...
    // בניית קונטיינר ראשי
    const mainContainer = document.getElementById('mainContainer') || document.querySelector('.main-content');
    if (mainContainer) {
        // נקה תוכן ישן
        ...
        newContainer.innerHTML = `
            <div class="table-header">
                <h2>בתי עלמין</h2>
                <button class="btn btn-primary" onclick="addNewCemetery()">
    ...
    try {
        // טען נתונים בשיטה הישנה והטובה - עם type!
        console.log('📡 Fetching cemeteries data...');
        const response = await fetch('/dashboard/dashboards/cemeteries/api/cemetery-hierarchy.php?action=list&type=cemetery&limit=100');
        ...
        if (data.success && data.data) {
            displayCemeteriesTable(data.data);
        } else {
            ...
            tbody.innerHTML = `
                <tr>
                    <td colspan="7" class="text-center text-danger">
                        ...
                        שגיאה בטעינת בתי עלמין: ${error.message}
                    </td>
                </tr>
            `;
        }
...
function displayCemeteriesTable(cemeteries) {
    console.log('📊 Displaying', cemeteries.length, 'cemeteries');
                ...
                <td colspan="7" style="text-align: center; padding: 40px;">
                    <div style="color: #999;">
                        ...
                        <button class="btn btn-primary mt-3" onclick="addNewCemetery()">
                            <i class="fas fa-plus"></i> הוסף בית עלמין ראשון
                        </button>
                    </div>
                </td>
            </tr>
    ...
    // בנה שורות טבלה
    tbody.innerHTML = cemeteries.map((cemetery, index) => {
        const cemeteryId = cemetery.unicId || cemetery.id;
        const cemeteryName = cemetery.cemeteryNameHe || cemetery.name || 'ללא שם';
        
        return `
            <tr data-id="${cemeteryId}" style="cursor: pointer;">
                <td>${index + 1}</td>
                <td>
                    <strong>${cemeteryName}</strong>
                    ${cemetery.cemeteryNameEn ? `<br><small class="text-muted">${cemetery.cemeteryNameEn}</small>` : ''}
                </td>
                <td>${cemetery.cemeteryCode || '-'}</td>
                <td>
                    ${cemetery.address || '-'}
                    ${cemetery.coordinates ? `<br><small class="text-muted">📍 ${cemetery.coordinates}</small>` : ''}
                </td>
                <td>
                    ${cemetery.contactName || '-'}
                    ${cemetery.contactPhoneName ? `<br><small class="text-muted">📞 ${cemetery.contactPhoneName}</small>` : ''}
                </td>
                <td>
                    ${cemetery.createDate ? new Date(cemetery.createDate).toLocaleDateString('he-IL') : '-'}
                </td>
                <td>
                    <button class="btn btn-sm btn-primary" 
                            onclick="event.stopPropagation(); openCemetery('${cemeteryId}', '${cemeteryName.replace(/'/g, "\\'")}')">
                        <i class="fas fa-sign-in-alt"></i> כניסה
                    </button>
                    <button class="btn btn-sm btn-secondary" 
                            onclick="event.stopPropagation(); editCemetery('${cemeteryId}')">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" 
                            onclick="event.stopPropagation(); deleteCemetery('${cemeteryId}')">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            </tr>
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
        const cemeteryId = tr.dataset.id;
        if (cemeteryId) {
            tr.ondblclick = () => {
                const cemetery = cemeteries.find(c => (c.unicId || c.id) === cemeteryId);
                if (cemetery) {
                    const name = cemetery.cemeteryNameHe || cemetery.name || 'ללא שם';
                    openCemetery(cemeteryId, name);
                }
            };
        }
    });
...
window.deleteCemetery = async function(cemeteryId) {
    ...
    try {
        ...
        if (result.success) {
            if (typeof showToast === 'function') {
                showToast('success', 'בית העלמין נמחק בהצלחה');
            }
            // רענן את הטבלה
            loadAllCemeteries();
        } else {
...
// פונקציית פתיחת בית עלמין - כבר קיימת ב-unified-table-renderer.js
// אבל נוסיף בדיקה אם היא לא קיימת
if (typeof window.openCemetery === 'undefined') {
    window.openCemetery = function(cemeteryId, cemeteryName) {
        console.log('🏛️ Opening cemetery:', cemeteryId, cemeteryName);
...
console.log('✅ Cemeteries loading fix applied successfully!');

// הפעל את הפונקציה החדשה אוטומטית אם הדף נטען
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        // בדוק אם זה דף בתי עלמין
        if (window.location.pathname.includes('/cemeteries')) {
            console.log('🚀 Auto-loading cemeteries...');
            setTimeout(() => loadAllCemeteries(), 100);
        }
    });
} else {
    // הדף כבר נטען
    if (window.location.pathname.includes('/cemeteries')) {
        console.log('🚀 Loading cemeteries immediately...');
        setTimeout(() => loadAllCemeteries(), 100);
    }
}