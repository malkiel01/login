#!/usr/bin/env bash
# קובץ: $HOME/public_html/form/login/scripts/deploy_customers_v2.1.0/rollback.sh
...
# שחזר customers-management.js
BACKUP_FILE=$(ls -t "$BACKUPS_DIR"/customers-management_backup_*_${VERSION}.js 2>/dev/null | head -1)

if [ -n "$BACKUP_FILE" ] && [ -f "$BACKUP_FILE" ]; then
    TARGET="$PROJECT_ROOT/dashboards/dashboard/cemeteries/js/customers-management.js"
    ...
    log "  ⚠️ לא נמצא גיבוי עבור customers-management.js"
fi
